...
很多人第一次学习 AI Infra，会不知不觉走成这样：

```text
看到 vLLM 很火 -> 去看 vLLM 参数
看到 SGLang 很火 -> 又去看 SGLang
看到 Triton / TensorRT-LLM -> 继续换方向
看到微调文章 -> 想先训练
看到 benchmark -> 开始比较模型
最后回到项目时，还是不知道一次请求怎么走。
```

这条路径不是完全错误，但它缺少主线。正确的顺序通常不是“哪个工具热门就先看哪个”，而是：

```text
系统地图 -> 最小运行 -> 输出证据 -> 单条主线 -> 小改动 -> 复盘
```

下面这些误区，本质上都是主线丢失后的表现。
...
判断自己是否太早追求生产级，可以看一个信号：你是否还解释不清当前 mock 版本的接口、事件和测试，却已经想加真实后端、数据库、鉴权中心和 dashboard。如果是，先退回最小闭环。
...
一个简单判断：如果问题是“模型如何生成 token”，优先看 inference；如果问题是“请求是否允许、路由到哪里、失败是否 fallback”，优先看 gateway。很多排障效率低，都是因为这一步没有先分清。
...
读任何榜单时，都建议先问一句：这个榜单的任务，和我当前系统要解决的问题是不是同一个问题？如果不是，它只能提供参考，不能直接变成发布判断。
...
微调前最值得补的通常不是训练脚本，而是评测集和数据说明。没有这两者，训练后你很难证明结果变好了，也很难解释为什么变坏。
...
如果你读完一页概念，最好马上找一个对应代码入口或输出文件。比如读完 request id，就去看 gateway response header 和 events；读完 checkpoint，就去看 checkpoint index。概念和证据贴得越近，理解越稳。
...
跑通一次只能证明环境和主链路大致可用。真正的学习通常发生在第二次：你改了一个小点，然后能解释输出为什么变了。
...
建议你主动制造一个小失败。比如错误 token、unknown model、同一个 eval 文件做 compare、错误 checkpoint 路径。能解释失败路径，才说明你真的理解系统边界。
...
公开发布意识越早建立，后面越省事。比如路径、示例数据、环境变量、截图、README 表述，都应该从一开始就按“别人会看到”来写。
...
如果你已经感觉有点乱，可以只做下面这个 30 分钟动作：

1. 打开系统地图。
2. 选 `inference-service` 或 `ai-gateway` 其中一个。
3. 跑一个成功请求。
4. 跑一个失败请求。
5. 找到对应 events 或 metrics。
6. 写三句话：现象、证据、判断。
