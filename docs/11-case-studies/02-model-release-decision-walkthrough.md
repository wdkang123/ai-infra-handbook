...
这类评审最容易犯的错误，是把“candidate 赢了”直接等价为“可以发布”。
...
如果前两步都不稳，后面就不该进入发布讨论。
如果前三步都稳，但没有观测和回滚，最多也只能进入更谨慎的试验阶段。
...
在看 compare 之前，建议先扫一遍 candidate 的低分样本。

你要问：

- 低分样本是不是集中在关键能力上？
- candidate 是否改变了输出格式？
- judge reason 是否显示同一种失败模式？
- 错误是否来自模型能力，还是来自 task / prompt / judge 配置？
- 这些样本在 baseline 里表现如何？

如果低分样本非常集中，平均分就要谨慎解读。

例如：

| 现象 | 判断 |
| --- | --- |
| 低分都在长上下文问题 | 可能是上下文处理退化 |
| 低分都在 JSON 格式 | 可能破坏下游解析 |
| 低分原因多为“答案过长” | 可能是成本和指令遵循问题 |
| baseline 也错同样样本 | 可能是数据或题目本身问题 |
...
```text
最终判断：candidate 不直接发布，进入 review。

当前证据支持：
- candidate 在相同 task 和 backend 下整体分数略高。
- run bundle 和 comparison report 都可追溯。
- leaderboard 显示 candidate 已进入可比较历史。

当前证据不足：
- 样本量仍然偏小。
- 低分样本中有格式不稳定现象。
- 尚未结合 gateway / inference 运行证据验证延迟和 token 成本。

主要风险：
- 如果直接发布，可能把格式不稳定带到下游。
- 分数提升可能不足以抵消输出变长带来的成本。

下一步：
- 人工复核 failed_sample_ids。
- 增加格式回归样本。
- 用相同设置重跑 candidate。
- 如果通过，再进入小流量灰度。
```

这段结论的价值在于：它不是只说“过/不过”，而是说明证据边界。
公开学习站里，读者最需要学的正是这种判断口径。
