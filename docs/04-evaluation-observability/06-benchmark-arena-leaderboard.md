...
假设你看到一个 leaderboard 里 candidate 排名第一。

不要立刻得出“它应该上线”的结论。先拆成三步：

1. 看任务：它在哪些 task 上赢了，哪些 task 上没有赢。
2. 看证据：能否回到 sample outputs、run metadata 和 comparison report。
3. 看代价：latency、cost、token usage、error rate 是否也可接受。

如果 candidate 只是某个综合分领先，但在你的关键任务上退化，那它不应该自动进入发布。

如果 candidate 在 benchmark 上提升，但 arena 偏好下降，也要分析原因：可能是回答更短、更保守、更不讨喜，也可能是 benchmark 与真实交互场景不匹配。
...
| 信号 | 结论倾向 |
| --- | --- |
| benchmark 提升，关键样本无退化，观测正常 | 可以进入下一步发布 review |
| benchmark 提升，但延迟/成本明显恶化 | 需要性能或路由策略复核 |
| arena 偏好提升，但事实性 benchmark 退化 | 不适合直接上线事实型任务 |
| leaderboard best 很高，但 latest 下降 | 先查配置、数据和 run history |
| 综合分提升，但关键业务样本失败 | block 或人工 review |

这个矩阵的重点是：不要让任何单一展示面替代多维证据。
...
公开榜单适合做候选集筛选，不适合直接做上线批准。

更稳的流程是：

1. 用公开 benchmark/arena 结果找到候选模型。
2. 用自己的 golden set 和真实任务重新评测。
3. 用 gateway/inference 观测检查延迟、错误和成本。
4. 用 compare 生成 release recommendation。
5. 记录为什么选择或拒绝某个候选。

这样外部榜单提供视野，内部 eval 提供决策证据。
