...
## 它和 learning inventory 的区别

可以这样区分：

| 产物 | 重点 |
| --- | --- |
| learning inventory | 站点有哪些内容、章节和主线 |
| course catalog | 这些内容如何组织成可学习模块 |

inventory 更像盘点表，catalog 更像课程设计稿。

如果你只是想知道“站点现在有哪些页面”，看 inventory。
如果你想带别人学、安排议程、做模块自测，看 catalog。
...
每个 module 不只是页面列表。它通常包含：

- audience：适合谁
- outcomes：学完能做什么
- entry：从哪一页进入
- grouped pages：阅读、练习、证据、自测、公开分享、生产迁移
- checkpoints：学习者应该交付或解释什么
- facilitator notes：讲师带练时注意什么

读 module 时先看 outcomes，再看 grouped pages。不要从页面数量开始判断模块质量。
...
| 问题 | 处理 |
| --- | --- |
| 模块只有概念页 | 补 lab、证据页或案例 |
| 模块没有自测 | 补 assessment 或 capstone 问题 |
| 模块入口太深 | 更新 overview 或 course catalog entry |
| 模块页面太多 | 拆成核心阅读和延伸阅读 |
| 讲师提示太少 | 从 workshop feedback 里补带练注意点 |

课程目录不是一次生成就结束。它应该随着内容变厚不断帮助你发现学习体验的失衡。
...
```text
本课程分成 7 个模块：
从 0 到 1、推理服务、平台治理、评测发布判断、训练资产复现、公开分享与共学、生产迁移。
每个模块都有入口页、核心阅读、动手练习、证据输出和自测/复盘。
```

这能让学习者知道自己不是在浏览散页，而是在走一条课程路径。
