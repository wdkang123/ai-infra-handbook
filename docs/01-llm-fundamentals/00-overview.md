...
这条链路里的每一个对象，都会在后面的模块里反复出现。Gateway 要记录 request id 和 usage，serving runtime 要管理 prefill/decode 和 KV Cache，eval 要保存输出样本，finetune 要知道训练数据如何变成后续模型行为。

所以这一章的目标不是“解释 LLM 是什么”，而是让你看到：LLM 在系统里会变成一组可以调度、计费、缓存、观测和评测的工程对象。
...
一个很实用的判断是：当你看到某个系统说“QPS 很高”时，先不要急着相信。继续问输入 token 和输出 token 分布是什么。10 个短请求和 10 个超长上下文请求，对 LLM serving 来说不是同一类负载。
...
在学习项目里，context 的影响不会像真实大模型那样直接消耗 GPU 显存，但你仍然应该从这里建立直觉。后面看到 gateway 做 request body 校验、serving 记录 usage、eval 保存 sample outputs、finetune 记录 dataset token 结构时，都可以回到这个问题：这段上下文到底被系统如何使用，如何限制，如何证明？
...
这也是为什么首 token 快不等于整体快。一个系统可能 TTFT 很好，但后续 token 间隔很不稳定；也可能 prefill 很慢，但 decode 之后比较平滑。真正排查用户体验时，需要把“等第一个字”和“后面持续输出”分开看。
...
不要把 token、context、prefill/decode、KV Cache 当成孤立概念。它们经常一起决定系统行为。

| 场景 | 主要受影响的概念 | 你应该观察什么 |
| --- | --- | --- |
| 用户输入很长 | context、prefill | 首 token 等待是否变长 |
| 用户要求长篇输出 | decode、KV Cache | token 间隔和请求占用时间 |
| 多个请求共享系统 prompt | prefix、KV Cache | prefix caching 是否可能有效 |
| 同一个 prompt 重复请求 | response cache / prefix cache | 命中对象是哪一层 |
| streaming 中途断开 | decode、events | 资源是否释放，错误如何记录 |
| 并发突然升高 | KV Cache、batching | 显存、队列和尾延迟 |

这张表是读 serving 工具页时的速查地图。很多工具差异，本质上都是在这些场景里做不同取舍。
...
读完这一章后，可以不改任何代码，只做一个观察练习：

1. 调一次普通 chat completion。
2. 调一次 streaming completion。
3. 看响应里的 `usage`。
4. 看 `/metrics` 里请求和错误指标。
5. 看 `/events` 或 request timeline。
6. 对照这章回答：哪些输出和 token、context、streaming、错误路径有关？

这个练习的重点不是验证模型质量，而是训练你把概念贴回系统证据。只要这一步走通，后面看 vLLM、SGLang 或 gateway 时就会稳很多。
...
以后你看到任何 serving 工具、模型 API 或性能报告，都可以先用这组问题过滤：

```text
它的输入输出单位是什么？
它如何描述 context 限制？
它如何处理 prefill 和 decode？
它是否暴露 TTFT / ITL / throughput？
它如何管理 KV Cache 或 prefix caching？
它如何表达 streaming 中途失败？
它的 metrics 能不能解释用户体验？
```

如果这些问题答不上来，说明你看到的可能只是营销层描述，还没有进入工程判断层。
