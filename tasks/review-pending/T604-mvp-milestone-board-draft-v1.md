...
# MVP Milestone Board Draft v1

## 概述

本文档给出 MVP 阶段化里程碑草案，按阶段划分，方便 Codex 后续拆解为更细粒度的任务。
...
| 任务 | 说明 | 验收条件 |
|------|------|---------|
| T2-01 | vLLM /metrics 端点 | metrics 可抓取 |
| T2-02 | Prometheus 部署 | metrics 存储可用 |
| T2-03 | Grafana + vLLM dashboard | dashboard 可查看 |
| T2-04 | Langfuse SDK 埋点 | trace 数据上报成功 |
| T2-05 | gateway tracing | token 用量可统计 |
...
| 任务 | 说明 | 验收条件 |
|------|------|---------|
| T4-01 | finetune-demo MVP 设计 | MVP 文档完成 |
| T4-02 | PEFT/LoRA 集成 | LoRA adapter 可训练 |
| T4-03 | QLoRA 集成（可选） | 4-bit 量化可训练 |
| T4-04 | trl SFTTrainer 集成 | SFT 可运行 |
| T4-05 | adapter 保存/加载 | adapter 持久化可用 |
| T4-06 | 训练+评测串联 | 微调后 benchmark 可跑 |
| T4-07 | Unsloth 加速（可选） | 加速效果可验证 |
...
| 任务 | 说明 | 验收条件 |
|------|------|---------|
| T5-01 | SGLang 备选引擎 | SGLang 可用 |
| T5-02 | Triton IS 集成 | 多模型编排可用 |
| T5-03 | TensorRT-LLM | 编译模型可部署 |
| T5-04 | Langfuse 结果上报 | 评测结果上报 Langfuse |
| T5-05 | DPO 微调 | DPO 可运行 |
| T5-06 | HumanEval benchmark | 代码评测可跑 |
