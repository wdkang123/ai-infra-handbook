...
| **TTFT** | Time To First Token | 首个 token 产出时间，受缓存影响 | https://docs.vllm.ai/ |
...
**目标**：使用 vLLM 验证 Prefix Caching 效果，对比有/无共享前缀的请求延迟差异。
...
# 2. 发送两个有共享前缀的请求
# 请求1：完整 prompt（包含长的 system prompt）
curl http://localhost:8000/v1/chat/completions \
  -H "Content-Type: application/json" \
  -d '{"messages":[{"role":"system","content":"[很长system prompt...]"},{"role":"user","content":"你好"}]}'

# 请求2：相同 system prompt，不同 user message
curl http://localhost:8000/v1/chat/completions \
  -H "Content-Type: application/json" \
  -d '{"messages":[{"role":"system","content":"[相同很长system prompt...]"},{"role":"user","content":"今天天气如何"}]}'

# 3. 对比两个请求的 TTFT
# 第二个请求的 TTFT 应明显更低（prefix cache 命中）
```
