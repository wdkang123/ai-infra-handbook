Task ID: T314
Task Title: 产出 finetuning glossary batch 04
Owner: MINIMAX
Status: REVIEW_PENDING

Input Summary:
基于 T207/T198/T209，产出 10 个 finetuning 相关术语的 glossary。
...
**定义**：Direct Preference Optimization，直接偏好优化，通过偏好数据（chosen/rejected）对）直接优化语言模型，无需单独的 reward model。
...
**定义**：专注于加速 LoRA/QLoRA 微调的开源项目，通过自研 CUDA kernel 实现训练速度提升，底层依赖 PEFT/TRL/PyTorch。

**边界说明**：Unsloth 是加速层，不改变 LoRA 方法本身；Unsloth 已被 TRL 集成作为可选后端，使用时无需改变训练代码结构。
