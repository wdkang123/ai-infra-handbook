# finetune-demo train.sh Blueprint v2
...
## Input Summary:
基于 T904 scaffold（run-script blueprint v1），产出 v2 版本训练脚本蓝图，增加 dry-run 和 env-file 加载支持。
...
本文档定义 `scripts/train.sh` 的 v2 蓝本是 T904 run-script blueprint 的细化版。
...
| 干跑 | `FINETUNE_DRY_RUN=true scripts/train.sh --dataset ./data/train.jsonl` |
