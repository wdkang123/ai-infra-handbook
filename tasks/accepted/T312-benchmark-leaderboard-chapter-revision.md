Task ID: T312
Task Title: 收紧 Benchmark / Leaderboard 章节初稿中的边界表述
Owner: MINIMAX
Status: REVIEW_PENDING

Input Summary:
基于 T311 初稿和 T311-review 修订。只收紧 Benchmark/Leaderboard/Arena 边界描述（第1节），并同步修正第2节和第7节。不重写整章。
...
Benchmark / Leaderboard / Arena 是 LLM 评测结果的不同形态，从执行到展示形成分层：

- **Benchmark**：在标准数据集上执行评测的体系，输出量化分数（如准确率、Pass@K），代表模型在特定任务上的绝对能力。包含评测执行和结果两部分。
- **Leaderboard**：聚合多个模型 Benchmark 结果的排名表，按分数排序，是 Benchmark 结果的展示聚合层。
- **Arena**：基于人类盲评对战的评测方式，通过 Elo 评分反映模型的相对强弱。包含对战机制和结果展示两部分。

**与 Evaluation 框架的边界**：Benchmark/Arena 包含执行机制，依赖底层 Evaluation 框架（如 LM-Eval Harness）提供评测能力。

## 2. 为什么重要

在 AI Infra 推理和选型中，Benchmark/Leaderboard/Arena 提供模型能力的客观参考：

1. **模型选型**：通过 Leaderboard 了解同规模模型的相对位置，通过 Arena 了解人类偏好视角
...
| 工具 | 定位 | 官方入口 |
|------|------|---------|
| **Open LLM Leaderboard** | 聚合开源模型 Benchmark 结果的排行榜 | https://huggingface.co/spaces/open-llm-leaderboard/open_llm_leaderboard |
| **LMSYS Chatbot Arena** | 基于人类盲评的模型对战排名 | https://chat.lmsys.org/ |
| **Stanford HELM** | 全面评测框架，执行 Benchmark 并展示结果 | https://crfm.stanford.edu/helm/ |
...
| **MMLU** | Massive Multitask Language Understanding | 多学科选择题综合评测 | https://github.com/EleutherAI/lm-evaluation-harness |
...
- eval-module 执行 Benchmark 评测后，结果可对标 Open LLM Leaderboard 等公开排名
- 了解各 Leaderboard 的评测维度和 Arena 的对战机制，有助于在模型选型时选择合适的参考基准
