...
## Input Summary:
基于 T805 shared config boundary，产出根目录共享 .env.example。
...
本文档定义仓库根目录的共享 `.env.example`，所有模块共享的环境变量在此定义。
...
## 端口分配（来自 T805 shared config boundary）
