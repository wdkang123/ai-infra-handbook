...
Input Summary:
基于 T195 初稿和 T195-review 修订。只修审阅指出的两个问题：命令链不一致、"70+"/"50+" 数量型表述。不重写整章。
...
| 工具 | 定位 | 官方入口 |
|------|------|---------|
| **LM-Eval Harness** | 标准 benchmark 评测工具，支持多种标准数据集 | https://github.com/EleutherAI/lm-evaluation-harness |
| **Stanford HELM** | 全面评测框架，多场景覆盖 | https://crfm.stanford.edu/helm/ |
...
**目标**：使用 LM-Eval Harness 配合 vLLM backend 直接运行一次 benchmark 评测，验证评测工具正常工作。

```bash
# 1. 安装 LM-Eval Harness 和 vLLM
pip install lm-eval[vllm] vllm

# 2. 直接运行评测（LM-Eval Harness 会自动启动 vLLM backend）
lm_eval \
  --model vllm \
...
- 最小实践路径是否与实际 lm_eval API 一致（vLLM backend 加载方式以官方文档为准）
