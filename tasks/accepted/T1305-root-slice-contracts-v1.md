# Root Integration Slice Contracts v1
...
# Root Integration Slice Contracts
...
**Cut Line：** 不写 CI/CD 目标，不写 Kubernetes 部署目标
...
**入口：** `bash scripts/local_dev_sequence.sh`
...
**前置条件：** R1 完成 + inference-service S2 + ai-gateway G3（路由+代理）

**完成信号：** 所有服务按顺序启动，健康检查全部返回 200

**Cut Line：** 不写定时重启脚本，不写日志收集
...
**前置条件：** R1 + 子项目 S2 + ai-gateway G3（路由+代理）完成
