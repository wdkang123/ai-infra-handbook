# Project Dependency Matrix v2 (Revised)

## Task ID: T714
## Task Title: Project Dependency Matrix Tighten
## Owner: MINIMAX
## Status: REVIEW_PENDING

## Input Summary:
基于 T704-review.md，修正 ai-gateway 依赖方向表述矛盾。
...
ai-gateway 作为入口层，接收外部用户请求。在项目内部，ai-gateway **依赖 inference-service 作为下游能力提供者**，通过 HTTP API 调用获取推理能力。

ai-gateway **不依赖其他项目模块作为上游**（用户请求直接到达 gateway，无需经过其他内部模块）。
