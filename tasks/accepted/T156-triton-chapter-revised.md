Task ID: T156
Task Title: 再修 Triton IS 章节中的最小实践和偏强表述
Owner: MINIMAX
Status: REVIEW_PENDING

Input Summary:
基于 T146 结果和 T146-review-round-2 修订。只改第 5 节关键指标和第 8 节最小实践两处。
...
- **吞吐（Throughput）**：Triton IS 自身作为服务层，开销通常小于后端推理本身；主要瓶颈在推理后端
...
**目标**：使用 Docker 快速启动 Triton IS 并部署一个 TensorFlow 分类模型，验证服务正常运行。

```bash
# 拉取含示例模型的 Triton 镜像（参考官方 QuickStart）
docker pull nvcr.io/nvidia/tritonserver:24.04-py3

# 创建模型仓库目录（参考官方文档结构）
mkdir -p model_repository/resnet50/1
# 注：官方 QuickStart 要求先准备模型文件，此处示例使用镜像内置的 cifar10 示例模型路径验证

# 启动服务（官方 QuickStart 方式）
docker run --rm --gpus=1 \
    -p 8000:8000 -p 8001:8001 -p 8002:8002 \
    nvcr.io/nvidia/tritonserver:24.04-py3 \
    tritonserver --model-repository=/opt/triton-inference-server/triton_repo

# 验证服务健康状态（官方 health endpoint）
curl -v http://localhost:8000/v2/health/ready
```

来源：https://docs.nvidia.com/deeplearning/triton-inference-server/user-guide/docs/getting_started/quickstart.html
...
7. https://docs.nvidia.com/deeplearning/triton-inference-server/user-guide/docs/getting_started/quickstart.html — 官方 QuickStart
...
- NVIDIA 已在 2026 年将 Triton Inference Server 品牌更名为 NVIDIA Dynamo-Triton，具体 API 和品牌可能调整
...
- Triton IS 已更名为 NVIDIA Dynamo-Triton，章节中的产品名称是否需要更新
