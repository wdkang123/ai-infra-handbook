# inference-service API Contract v1 (Revised)

## Task ID: T811
## Task Title: inference-service API Contract Tighten
## Owner: MINIMAX
## Status: REVIEW_PENDING

## Input Summary:
基于 T801-review.md，把 `/v1/completions`、`/v1/models` 从 MVP 必须降级为可选。
...
| `/v1/completions` | POST | 文本补全 | 否（后续扩展） |
...
| 兼容性项 | 状态 | 说明 |
|---------|------|------|
| `/v1/chat/completions` | 兼容 | vLLM 内置支持，MVP 核心 |
| `/v1/completions` | 兼容（可选） | vLLM 内置支持，后续扩展 |
| `/v1/models` | 需实现（可选） | 后续扩展 |
| 流式输出 | 兼容 | vLLM SSE 支持 |
| Token 计算 | 兼容 | vLLM 内置 |
