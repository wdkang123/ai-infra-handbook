Task ID: T204
Task Title: 收紧 Observability 章节初稿中的指标来源和最小实践
Owner: MINIMAX
Status: REVIEW_PENDING

Input Summary:
基于 T194 初稿和 T194-review 修订。只收紧第 5 节来源、第 8 节最小实践、第 10 节延伸阅读，不重写整章。
...
| 指标 | 全称 | 说明 | 来源 |
|------|------|------|------|
| **TTFT** | Time To First Token | 首个 token 产出时间，反映 prefill 效率 | https://docs.vllm.ai/ |
| **ITL** | Inter-Token Latency | 相邻 token 产出间隔，反映 decode 效率 | https://docs.vllm.ai/ |
| **E2E Latency** | End-to-End Latency | 请求到响应的完整延迟 | https://langfuse.com/docs/observability/overview |
| **TPM** | Tokens Per Minute | 每分钟 token 消耗 | https://docs.nvidia.com/deeplearning/triton-inference-server/user-guide/docs/ |
| **RPS** | Requests Per Second | 每秒请求数 | https://langfuse.com/docs/observability/overview |
| **P99 Latency** | 99th Percentile Latency | 99% 请求的最大延迟 | https://langfuse.com/docs/observability/overview |
...
**目标**：使用 Langfuse 采集 vLLM 请求的 traces，验证 trace 数据正常采集和上报。
...
- vLLM 请求的 trace span 数据上报到 Langfuse
- 可在 dashboard 查看单个请求的调用链和环节耗时
...
5. https://grafana.com/ — Grafana（指标可视化）

注：Evaluation 相关工具（LM-Eval Harness、Stanford HELM）属于评测范畴，本章不做展开，只在第 1 节术语边界中做区分。
...
6. https://docs.vllm.ai/ — vLLM 指标文档
7. https://docs.nvidia.com/deeplearning/triton-inference-server/user-guide/docs/ — Triton IS 指标文档
